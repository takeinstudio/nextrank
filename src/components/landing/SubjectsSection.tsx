...
// Pyramid: 3 on top row, 4 on bottom row
const row1 = SUBJECT_CARDS.slice(0, 3);
const row2 = SUBJECT_CARDS.slice(3);

const SubjectCard = ({ subjectCard, index }: { subjectCard: typeof SUBJECT_CARDS[number]; index: number }) => (
  <motion.div
  ...
  <section className="py-16 md:py-24 px-4 gradient-bg">
    <div className="container max-w-5xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
        {row1.map((card, i) => <SubjectCard key={card.name} subjectCard={card} index={i} />)}
      </div>

      {/* Row 2 — 4 cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
        {row2.map((card, i) => <SubjectCard key={card.name} subjectCard={card} index={row1.length + i} />)}
      </div>
    </div>
  </section>
);
